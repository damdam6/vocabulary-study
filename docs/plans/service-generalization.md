...
>
> 작성일: 2026-07-21 · 상태: **초안** — PRD-general §10 미결 질문(특히 Q1)이 코드 착수 게이트다. Q1(시트 단위) 답이 기본값과 다르면 작업 1·2의 설계가 바뀐다.
...
- 목표: Worker 1개·SPA 1개·시트 컬럼 계약을 유지한 채 프로필 차원만 추가한다. 학습 모델은 "활성 모드 집합 M으로 파라미터화"가 전부이고, M = {m1,m2}일 때 기존 동작과 완전히 일치해야 한다 (기존 테스트가 그대로 통과하는 것이 그 증명).
- 비목표: 회원가입·멀티테넌트 SaaS화, 콘텐츠 타입 일반화(PRD-general §10 Q4 확정 전), SRS 알고리즘 변경, 화면 구조 변경.
...
| 파일 | 변경 |
|---|---|
| `worker/lib/profiles.ts` **(신규)** | `PROFILES` 파싱·검증(중복 password/id·빈 modes 거부)·isolate 캐시, `APP_PASSWORD`+`SHEET_ID` 폴백 합성. 구성 오류는 구분 가능한 에러로 던져 index.ts가 500으로 변환 |
...
| `src/screens/HomeScreen.tsx` | words 응답의 프로필 사용(이름 표기·저장본 갱신·modes를 집계/큐에 전달), "프로필 전환" 링크 |
| `src/App.tsx` | 프로필 전환 핸들러(저장값 삭제 → login 화면) |
| `src/lib/studySession.ts` · `StudyScreen` · `Mode1Card` · `Mode2Card` · `DoneScreen` | **불변** — 출제 모드는 큐가 이미 결정한 상태로 내려온다 |
| `src/screens/RegisterScreen.tsx` · 등록 검증 일체 | **불변** (Q4 확정 전 — 대상 시트만 프로필로 자동 결정됨) |
...
| `docs/design-prd.md` | §2 로그인(프로필 저장)·§3 홈(이름 표기·전환 링크) 개정 — 작업 4에 포함, Q5 확정 후 |
...
| # | 내용 | 산출물 | 의존 |
|---|---|---|---|
| 0 | **PRD 확정** — 미결 질문 Q1~Q5 해소, PRD-general·이 플랜 개정 후 상태 확정 | 문서 2건 확정 | — |
...
| 4 | **클라이언트 프로필 컨텍스트** — api/wordsApi 프로필 수신·저장, Login/Home/App 배선, 프로필 이름 표기·전환 링크, design-prd §2·§3 개정 | 화면 개정 + 문서 | 2, 3 |
| 5 | **재시도 큐 프로필 태깅** — profileId 태깅·필터 flush·레거시 승격·4xx 폐기 | retryQueue 개정 + 테스트 | 4 |
| 6 | **운영 전환·정리** — `PROFILES` 설정(1번 프로필 = 현 값), 새 프로필 시트 생성·서비스 계정 공유, 안정화 후 구 변수·폴백 제거 | 운영 체크리스트 실행 + 정리 PR | 4·5 |

의존 그래프: `0 → 1 → 2 → 4 → 5 → 6`, `0 → 3 → 4`. 각 작업은 단독 배포 가능하게 자른다 — 1·2가 머지돼도 폴백 프로필로 기존 동작이 유지되고, 3은 M = {m1,m2} 고정 호출로 시작해 4에서 실제 modes가 연결된다.
...
PRD-general §10의 Q1~Q5가 원본이다. 코드 영향으로 번역하면:

- **Q1 (시트 단위)** — 기본값(스프레드시트)이 뒤집혀 "한 스프레드시트 내 탭 부분집합"이 되면: `profiles.ts`의 구성 필드(sheetId → 탭 선택 규칙), words/tabs/register의 탭 필터링, 등록 화면의 새 탭 생성 정책이 전부 재설계된다. 작업 1·2 착수 전에 반드시 확정.
- **Q2 (구성 저장 위치)** — 시크릿이 아니면 작업 1에 설정 로더가 추가된다.
- **Q3 (졸업 조건)** — 임계값 구성이 붙으면 작업 1(스키마)·3(판정)에 반영.
- **Q4 (비중국어 콘텐츠)** — v-next 범위 밖 유지가 기본값. 도입 시 별도 플랜으로 분리 권장 (등록 검증·B열 의미·폰트 처리까지 번지는 독립 주제).
- **Q5 (홈 프로필 표기)** — 작업 4의 design-prd 개정에서 확정하면 충분.
...
| 등록 화면 | 이번 범위에서 불변 | 프로필별 노출 플래그(Q4 확정 전 과설계) |
