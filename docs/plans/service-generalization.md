...
>
> 작성일: 2026-07-22 · 상태: **확정** — 같은 날 소유자 확인으로 PRD-general §10 Q1~Q8 전부 해소, **코드 착수 가능**. 잔여 미결 없음 (홈 표기의 구체 스타일 값만 작업 4의 design-prd 개정에서 정한다).
...
| # | 내용 | 산출물 | 의존 |
|---|---|---|---|
| 0 | ~~**PRD 확정**~~ — **완료** (2026-07-22 소유자 확인 Q1~Q8, 문서 개정 반영) | 문서 2건 확정 | — |
...
## 7. 질문 해소 현황 (2026-07-22 소유자 확인 — PRD-general §10이 원본, 잔여 미결 없음)
...
- **Q5 (홈 프로필 표기)**: **타이틀 아래 저강조 확정** → 작업 4는 확정값 반영, design-prd에는 구체 스타일 값만 남음.
- **Q6 (generic 모드 2)**: **모드 1만 쓸 예정** → 채점 분기 없음 — 작업 3·6 어디에도 느슨 채점(대소문자 무시) 작업이 들어가지 않는다. studySession의 `gradeMode2` 불변 확정.
- **Q7 (모드 확대 동작)**: **수용** → 보호 장치(졸업 면제 기록) 작업 없음.
- **Q8 (전환 UX)**: **비밀번호 재입력** → 다중 프로필 기억 UI 없음, 작업 4 범위 그대로.
