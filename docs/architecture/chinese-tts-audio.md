...
| 500 | 기존 `PROFILES` 구성 오류 또는 예상 못 한 내부 오류 | 기존 인증 오류 계약 또는 일반 TTS 실패 |
...
`index.ts`의 fetch에 `ctx`를 추가하고 필요한 라우트에 전달한다. 기존 테스트에서 직접 `worker.fetch(request, env)`를 호출하던 곳은 실행 컨텍스트 테스트 더블을 받도록 갱신한다.
