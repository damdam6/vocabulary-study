/**
 * 기록 유실 방지 재시도 큐 (PRD §10, design-prd §3 미전송 인디케이터, 이슈 #18·#43).
...
import { postAnswer, postReviewFail, type AnswerRecord, type ReviewFailRecord } from "./api.ts";
...
export type RetryQueueEntry =
  ...
  | { kind: "review-fail"; record: ReviewFailRecord };
 ...
 * 대상에서 조용히 빠지고 유실될 수 있다(#18의 유실 방지 취지 위반).
 */
function readQueue(): RetryQueueEntry[] {
...
/**
 * 큐를 FIFO로 재전송한다. 엔트리 kind에 따라 postAnswer/postReviewFail로 분기
 * 호출한다. 항목은 전송 성공 시에만 제거하고, 첫 실패에서 중단해 잔여를 유지한다
 * (네트워크가 죽어 있으면 나머지도 실패할 것이므로). 전송 중 새로 적재된 항목은
 * 뒤에 append되므로(제거는 재읽기 후 맨 앞) 유실되지 않는다.
 */
