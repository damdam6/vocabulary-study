 ...
 * 둔다. 홈 화면 현황 집계와 학습 세션 큐 구성 양쪽에서 이 함수를 그대로 재사용한다.
 */
...
/** 출제 모드. PRD-general §3의 프로필 `modes` 원소와 동일 표기. */
export type Mode = "m1" | "m2";
...
/**
 * PRD-general §4.1 상태 판정 — 활성 모드 집합 `modes`(M) 기준. `today`는 호출자가
 * getSeoulToday()로 한 번 계산해 넘긴다 — 홈 집계·세션 큐 구성 모두 여러 단어를
